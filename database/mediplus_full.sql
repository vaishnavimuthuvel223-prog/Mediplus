...
-- FULL MERGED DATABASE (single-file install)
--
-- This file replaces the old two-file setup
-- (schema.sql + migration_token_travel.sql). Everything the
-- migration used to add (booking_type, consultation timer,
-- travel reminders, walk-in tokens, hospital location
-- settings) is already included below in its correct place.
--
-- Use this file for ALL fresh installs from now on:
--   mysql -u root -p < mediplus_full.sql
-- or import it directly through phpMyAdmin.
--
-- Do NOT also import schema.sql or migration_token_travel.sql
-- alongside this file - it already contains both.
-- =========================================================

DROP DATABASE IF EXISTS mediplus;
CREATE DATABASE mediplus CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE mediplus;
...
-- ---------------------------------------------------------
-- PATIENTS  (includes geocoded location columns from the
-- old travel migration)
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- APPOINTMENTS (includes booking_type, added_by_admin_id,
-- consultation_started_at, travel_reminder_sent - all from
-- the old token/travel migration, already merged in here)
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- TOKEN PROXIMITY REMINDERS (from the old migration file -
-- one row per appointment/threshold so a patient is never
-- notified twice for the same milestone)
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- SETTINGS (configurable values, includes hospital location
-- + reminder/travel config from the old migration file)
-- ---------------------------------------------------------
